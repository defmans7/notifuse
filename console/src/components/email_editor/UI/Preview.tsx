...
export const json2mjml = (node: any, indent = 0) => {
  let { tagName } = node
  ...
  const inside =
    (content && `\n${space}  ${content}\n${space}`) ||
    (children &&
      `\n${children.map((c: any) => `${json2mjml(c, indent + 2)}`).join('\n')}\n${space}`) ||
    ''
...
export const treeToMjmlJSON = (
  ...
  switch (block.kind) {
    ...
    case 'heading':
      ...
      if (
        ...
        try {
          ...
          // First, check if content contains any Nunjucks filter syntax that might be incompatible with Liquid
          // Nunjucks uses '|filter(arg)' syntax, while Liquid uses '|filter:arg'
          if (content.includes('|') && /\|\s*\w+\s*\(/.test(content)) {
            console.warn(
              'Detected potential Nunjucks filter syntax that might be incompatible with Liquid:',
              content.match(/\{\{[^}]*\|[^}]*\}\}/g) || []
            )
          ...
          // console.log('Rendering content with Liquid:', content)
          const stringResult = engine.parseAndRenderSync(content, jsonData || {})
          content = stringResult
        } catch (e: any) {
          // Log detailed error info to help debug
          console.error('Liquid rendering error:', e)
          console.error('Content causing error:', content)
          console.error('Data:', jsonData)

          // Return the unprocessed content to avoid breaking the editor
          // Optionally add error indicator in the template for debugging
    ...
    case 'liquid':
      let processedLiquidContent = ''

      try {
        // Parse and render the liquid code with the provided template data
        const engine = new Liquid()
        const jsonData = templateData ? JSON.parse(templateData) : {}

        // Convert the liquid code to HTML/MJML
        processedLiquidContent = engine.parseAndRenderSync(block.data.liquidCode, jsonData)
      } catch (e: any) {
        console.error('Liquid rendering error in liquid block:', e)
        processedLiquidContent = `<!-- Liquid error: ${e.message} -->`
      }

      // Default behavior: wrap in mj-text
      return {
        tagName: 'mj-liquid',
        attributes: {
          // align: block.data.align,
          // 'container-background-color': block.data.backgroundColor || undefined,
          padding: 0
        },
        content: processedLiquidContent
      }
