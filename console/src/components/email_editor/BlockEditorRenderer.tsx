import { CSSProperties, MouseEvent, ReactNode, useRef, RefObject, memo } from 'react'
...
export interface BlockEditorRendererProps {
  ...
  onFocusBlock: (node: RefObject<HTMLDivElement | null> | undefined) => void
...
export const BlockEditorRenderer = memo((props: BlockEditorRendererProps) => {
  ...
  if (!blockDefinition) {
    return (
      <div className="xpeditor-block-content">Block definition missing for {props.block.kind}</div>
    )
  }
  if (!blockDefinition.renderEditor) {
    return (
      <div className="xpeditor-block-content">
        ...
        {props.block.children?.map((_childBlock, i) => {
          ...
          return (
            <Container
              ...
              dragHandleSelector=".xpeditor-drag-handle"
              style={containerStyle}
              dragClass="xpeditor-ghost-drag"
              dropClass="xpeditor-ghost-drop"
              dropPlaceholder={{
                animationDuration: 200,
                showOnTop: true,
                className: 'xpeditor-drop-preview'
              }}
              // onDragEnter={() => {
  ...
  return (
    <div
      ...
      id={props.block.id}
      className={cn({
        'xpeditor-block-content': props.block.kind !== 'root',
        // 'focused': props.focusedBlock && props.focusedBlock.id === props.block.id,
        'xpeditor-selected': props.block.id === props.selectedBlockId
      })}
    ...
    >
      {props.block.id !== 'root' && props.block.id === props.selectedBlockId && (
        <span
          className="xpeditor-drag-handle"
        ...
        >
          {props.renderSelectedBlockButtons({
            ...
            deleteBlock: props.deleteBlock,
            cloneBlock: props.cloneBlock,
            existingBlocks: [], // This will be overridden by the wrapper in Editor.tsx
            onExistingBlocksUpdate: async () => Promise.resolve() // This will be overridden by the wrapper in Editor.tsx
          })}
        </span>
      )}
